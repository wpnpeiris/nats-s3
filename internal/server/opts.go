...
func ConfigureOptions(fs *flag.FlagSet, args []string, printVersion, printHelp func()) (*Options, error) {
	...
	fs.StringVar(&opts.User, "user", "", "NATS server username (basic auth)")
	fs.StringVar(&opts.Password, "password", "", "NATS server password (basic auth)")
	fs.StringVar(&opts.Token, "token", "", "NATS server token (token auth)")
	fs.StringVar(&opts.NkeyFile, "nkeyFile", "", "NATS server NKey seed file path (nkey auth)")
	fs.StringVar(&opts.CredsFile, "credsFile", "", "NATS server credentials file path (JWT auth)")

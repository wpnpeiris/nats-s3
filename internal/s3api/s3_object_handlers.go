...
// Returns object retention configuration (mode and retain-until-date)
...
// Sets object retention configuration (mode and retain-until-date)
func (s *S3Gateway) PutObjectRetention(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	key := vars["key"]
